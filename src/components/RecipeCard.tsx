...
export default function RecipeCard({ recipe }: { recipe: Recipe }) {
  ...
  return (
    <article className="group overflow-hidden rounded-2xl border border-orange-100 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-md sm:rounded-3xl">
      <Link href={`/recipes/detail?id=${encodeURIComponent(recipe.id)}`} className="block"><div className="relative h-44 overflow-hidden sm:h-48"><img src={recipe.image} alt={recipe.title} className="h-full w-full object-cover transition duration-300 group-hover:scale-105" /><div className="absolute left-3 top-3 rounded-full bg-white/90 px-3 py-1 text-xs font-bold text-orange-500">{recipe.category}</div></div></Link>
      <div className="p-4">
