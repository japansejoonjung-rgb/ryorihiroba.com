...
export default function RecipeCard({ recipe }: { recipe: Recipe }) {
  ...
  return (
    <article className="group overflow-hidden rounded-3xl border border-orange-100 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-md">
      <Link href={`/recipes/detail?id=${encodeURIComponent(recipe.id)}`} className="block"><div className="relative h-48 overflow-hidden"><img src={recipe.image} alt={recipe.title} className="h-full w-full object-cover transition duration-300 group-hover:scale-105" /><div className="absolute left-3 top-3 rounded-full bg-white/90 px-3 py-1 text-xs font-bold text-orange-500">{recipe.category}</div></div></Link>
      <div className="p-4">
        <Link href={`/recipes/detail?id=${encodeURIComponent(recipe.id)}`}><h3 className="text-lg font-bold text-stone-800 transition hover:text-orange-500">{recipe.title}</h3></Link>
        ...
        <div className="mt-4 flex items-center justify-between border-t border-orange-50 pt-4">
          ...
          <div className="flex items-center gap-2">
            <button type="button" onClick={handleLike} className={`flex items-center gap-1 rounded-full px-3 py-1 text-xs font-bold ${liked ? "bg-rose-50 text-rose-500" : "bg-stone-50 text-stone-500 hover:bg-rose-50 hover:text-rose-500"}`}><Heart size={15} fill={liked ? "currentColor" : "none"} />{likeCount}</button>
            <button type="button" onClick={handleSave} className={`rounded-full p-2 ${saved ? "bg-orange-100 text-orange-500" : "bg-stone-50 text-stone-500 hover:bg-orange-50 hover:text-orange-500"}`}><Bookmark size={15} fill={saved ? "currentColor" : "none"} /></button>
          </div>
        </div>
      </div>
    </article>
