...
export default function RankingList({ title, recipes, type }: Props) {
  const getValue = (recipe: Recipe) => {
    if (type === "views") return `${recipe.views.toLocaleString()} views`;
    if (type === "likes") return `${recipe.likes.toLocaleString()} likes`;
    return `${recipe.saves.toLocaleString()} saves`;
  };
    ...
    <section className="rounded-3xl border border-orange-100 bg-white p-5 shadow-sm">
      ...
      <div className="mt-5 grid gap-4">
        {recipes.map((recipe, index) => (
          <Link key={recipe.id} href={`/recipes/${recipe.id}`} className="flex gap-4 rounded-2xl p-2 transition hover:bg-orange-50">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1 text-sm text-stone-500">{recipe.category}</p>
